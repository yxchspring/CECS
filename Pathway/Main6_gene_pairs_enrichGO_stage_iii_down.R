...
for(pcc_cutoff_item in PCC_cutoff){
  ...
  for(project in projects[c(2:9)]){
    ...
    DEGs_len_exper <- dim(CEG_Matrix_Stage_III_down)[1]
    
    ### the matrix form
    # CEG_Matrix_N_III
    # CEG_Matrix_Stage_III
    ### the vector form
    exper_III_down <- which(CEG_Matrix_Stage_III_down == 100)
    
    ### the coming result step: the length of the non zero term is :
    N_exper <- DEGs_len_exper * (DEGs_len_exper)/2
    K_exper <- length(exper_III_down)
    ...
    for(qq in 1: dim(enrichGO_result_III.result)[1]){
      ...
      ### get sub reference index matrix for the exper data: CEG_Matrix_Stage_III_down
      CEG_Matrix_Stage_III_down_each <- CEG_Matrix_Stage_III_down[geneID_each,geneID_each]
      len_each <- length(geneID_each)
      
      n_exper <- len_each*(len_each+1)/2
      # k_exper <- length(which(is.na(CEG_Matrix_Stage_III_down_each)==FALSE))
      k_exper <-length(which(CEG_Matrix_Stage_III_down_each >=100))
      # M <- K
      # m+n <- N
      # x <- k
      # k <- n
    ...
    save(GO_term_exper,GenePairs_GO_term_exper,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_iii/",project,"_gene_pairs_GOterms_exper_085_stageiii_down.Rdata",sep = ""))
  ...
  write.xlsx(Genepairs_xlsx_exper_result,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_iii/","List_gene_pairs_GOterms_exper_085_stageiii_down.xlsx",sep = ""))
