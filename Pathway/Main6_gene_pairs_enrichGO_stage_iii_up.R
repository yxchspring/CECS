...
for(pcc_cutoff_item in PCC_cutoff){
  ...
  for(project in projects[c(2:9)]){
    ...
    load(paste("./main5_enrichGO_results_Initial_up/",project,"_enrichGO_results.Rdata",sep = ""))
    ...
    DEGs_len_exper <- dim(CEG_Matrix_Stage_III_up)[1]
    
    ### the matrix form
    # CEG_Matrix_N_III
    # CEG_Matrix_Stage_III
    ### the vector form
    exper_III_up <- which(CEG_Matrix_Stage_III_up == 100)
    
    ### the coming result step: the length of the non zero term is :
    N_exper <- DEGs_len_exper * (DEGs_len_exper)/2
    K_exper <- length(exper_III_up)
    # NK_exper <- c(N_exper,K_exper) ################################################## backgroud
    NK_exper <- paste(K_exper,"/",N_exper,sep = "") ################################################## backgroud
    
    #################### Step 3: construct the experimetal information using the enrichGO_result_III.result daat
    GO_pvalue_exper <- list()
    nk_exper <- list()
    GOcount_exper <- list()
    padjust_exper <- NULL
    count_exper <- 0
    for(qq in 1: dim(enrichGO_result_III.result)[1]){
      count_exper <- count_exper + 1
      ### get the corresponding geneID for the term qq
      geneID_each <- unlist(strsplit(enrichGO_result_III.result$geneID[qq] ,split="/"))
      ### get sub reference index matrix for the exper data: CEG_Matrix_Stage_III_up
      CEG_Matrix_Stage_III_up_each <- CEG_Matrix_Stage_III_up[geneID_each,geneID_each]
      len_each <- length(geneID_each)
      
      n_exper <- len_each*(len_each+1)/2
      # k_exper <- length(which(is.na(CEG_Matrix_Stage_III_up_each)==FALSE))
      k_exper <-length(which(CEG_Matrix_Stage_III_up_each >=100))
      # M <- K
      # m+n <- N
      # x <- k
      # k <- n
    ...
    goterm_len <- dim(enrichGO_result_III.result)[1]
    GPRatio_exper <- unlist(nk_exper)
    BgRatio_exper <- rep(NK_exper,goterm_len)
    # padjust_exper
    GOcount_exper <- unlist(GOcount_exper)
    GenePairs_enrichGO_result_III.result_exper <- NULL
    GenePairs_enrichGO_result_III.result_exper <- data.frame("ID" = enrichGO_result_III.result$ID,
                                                          "Description" = enrichGO_result_III.result$Description,
                                                          "GPRatio" = GPRatio_exper,
                                                          "BgRatio" = BgRatio_exper,
                                                          "pvalue" = GO_pvalue_exper,
                                                          "p.adjust" = padjust_exper,
                                                          "GOcount" = GOcount_exper)
    rownames(GenePairs_enrichGO_result_III.result_exper) <- GenePairs_enrichGO_result_III.result_exper$ID
    
    GO_term_exper <- enrichGO_result_III.result[which(GO_pvalue_exper < 0.05),]
    GenePairs_GO_term_exper <- GenePairs_enrichGO_result_III.result_exper[which(GO_pvalue_exper < 0.05),]
    GenePairs_GO_term_exper <- GenePairs_GO_term_exper[order(GenePairs_GO_term_exper$pvalue),]
    save(GO_term_exper,GenePairs_GO_term_exper,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_iii/",project,"_gene_pairs_GOterms_exper_085_stageiii_up.Rdata",sep = ""))
  ...
  write.xlsx(Genepairs_xlsx_exper_result,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_iii/","List_gene_pairs_GOterms_exper_085_stageiii_up.xlsx",sep = ""))
