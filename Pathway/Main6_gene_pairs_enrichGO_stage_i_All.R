...
for(pcc_cutoff_item in PCC_cutoff){
  ...
  for(project in projects[c(2:9)]){
    ...
    for(qq in 1: dim(enrichGO_result_I.result)[1]){
      ...
      ### get sub reference index matrix for the ctrl data: CEG_Matrix_N_I
      CEG_Matrix_N_I_each <- CEG_Matrix_N_I[geneID_each,geneID_each]
      len_each <- length(geneID_each)
      
      n_ctrl <- len_each*(len_each+1)/2
      # k_ctrl <- length(which(is.na(CEG_Matrix_N_I_each)==FALSE))
      k_ctrl <-length(which(CEG_Matrix_N_I_each >=100))
      # M <- K
      # m+n <- N
      # x <- k
      # k <- n
    ...
    GenePairs_enrichGO_result_I.result_ctrl <- NULL
    GenePairs_enrichGO_result_I.result_ctrl <- data.frame("ID" = enrichGO_result_I.result$ID,
    ...
    rownames(GenePairs_enrichGO_result_I.result_ctrl) <- GenePairs_enrichGO_result_I.result_ctrl$ID
    
    GO_term_ctrl <- enrichGO_result_I.result[which(GO_pvalue_ctrl < 0.05),]
    GenePairs_GO_term_ctrl <- GenePairs_enrichGO_result_I.result_ctrl[which(GO_pvalue_ctrl < 0.05),]
    GenePairs_GO_term_ctrl <- GenePairs_GO_term_ctrl[order(GenePairs_GO_term_ctrl$pvalue),]
    save(GO_term_ctrl,GenePairs_GO_term_ctrl,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_i/",project,"_gene_pairs_GOterms_ctrl_085_stagei.Rdata",sep = ""))
  ...
  write.xlsx(Genepairs_xlsx_ctrl_result,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_i/","List_gene_pairs_GOterms_ctrl_085_stagei.xlsx",sep = ""))
