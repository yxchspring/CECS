...
for(pcc_cutoff_item in PCC_cutoff){
  ...
  for(project in projects[c(2:9)]){
    ...
    DEGs_len_exper <- dim(CEG_Matrix_Stage_I_up)[1]
    
    ### the matrix form
    # CEG_Matrix_N_I
    # CEG_Matrix_Stage_I
    ### the vector form
    exper_I_up <- which(CEG_Matrix_Stage_I_up == 100)
    
    ### the coming result step: the length of the non zero term is :
    N_exper <- DEGs_len_exper * (DEGs_len_exper)/2
    K_exper <- length(exper_I_up)
    ...
    for(qq in 1: dim(enrichGO_result_I.result)[1]){
      ...
      ### get sub reference index matrix for the exper data: CEG_Matrix_Stage_I_up
      CEG_Matrix_Stage_I_up_each <- CEG_Matrix_Stage_I_up[geneID_each,geneID_each]
      len_each <- length(geneID_each)
      
      n_exper <- len_each*(len_each+1)/2
      # k_exper <- length(which(is.na(CEG_Matrix_Stage_I_up_each)==FALSE))
      k_exper <-length(which(CEG_Matrix_Stage_I_up_each >=100))
      # M <- K
      # m+n <- N
      # x <- k
      # k <- n
    ...
    save(GO_term_exper,GenePairs_GO_term_exper,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_i/",project,"_gene_pairs_GOterms_exper_085_stagei_up.Rdata",sep = ""))
  ...
  write.xlsx(Genepairs_xlsx_exper_result,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_i/","List_gene_pairs_GOterms_exper_085_stagei_up.xlsx",sep = ""))
