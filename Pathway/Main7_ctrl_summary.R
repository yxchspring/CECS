...
for(pcc_cutoff_item in PCC_cutoff){
  path_ctrl_regulated <- paste0("./085/",pcc_cutoff_item,"/","generalized_Ctrl/")
  
  ########################## step 1: load the data
  file_name_ctrl <- list.files(path_ctrl_regulated,pattern = "xlsx$")
  
  ctrl_regulated_list <- list()
  projects_count <- 0
  for (ii in projects[2:9]) {
    ...
    file_count <- 0
    for (jj in file_name_ctrl) {
      file_count <- file_count + 1
      stage_list_type[[file_count]] <- read.xlsx(paste(path_ctrl_regulated,jj,sep = ""),sheet =ii)
    ...
    ctrl_regulated_list[[projects_count]] <- df_list_type
    cat("The project ",ii," is completed!","\n")
    
  }
  names(ctrl_regulated_list) <- projects[2:9]
  # write.xlsx(ctrl_regulated_list,file = "./generalized_Ctrl_summary.xlsx")
  write.xlsx(ctrl_regulated_list,file = paste0("./085/",pcc_cutoff_item,"/","generalized_Ctrl_summary_",pcc_cutoff_item,".xlsx"))
