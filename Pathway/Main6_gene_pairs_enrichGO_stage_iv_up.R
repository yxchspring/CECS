...
for(pcc_cutoff_item in PCC_cutoff){
  ...
  for(project in projects[c(2:9)]){
    ...
    DEGs_len_exper <- dim(CEG_Matrix_Stage_IV_up)[1]
    
    ### the matrix form
    # CEG_Matrix_N_IV
    # CEG_Matrix_Stage_IV
    ### the vector form
    exper_IV_up <- which(CEG_Matrix_Stage_IV_up == 100)
    
    ### the coming result step: the length of the non zero term is :
    N_exper <- DEGs_len_exper * (DEGs_len_exper)/2
    K_exper <- length(exper_IV_up)
    ...
    for(qq in 1: dim(enrichGO_result_IV.result)[1]){
      ...
      ### get sub reference index matrix for the exper data: CEG_Matrix_Stage_IV_up
      CEG_Matrix_Stage_IV_up_each <- CEG_Matrix_Stage_IV_up[geneID_each,geneID_each]
      len_each <- length(geneID_each)
      
      n_exper <- len_each*(len_each+1)/2
      # k_exper <- length(which(is.na(CEG_Matrix_Stage_IV_up_each)==FALSE))
      k_exper <-length(which(CEG_Matrix_Stage_IV_up_each >=100))
      # M <- K
      # m+n <- N
      # x <- k
      # k <- n
    ...
    GenePairs_enrichGO_result_IV.result_exper <- NULL
    GenePairs_enrichGO_result_IV.result_exper <- data.frame("ID" = enrichGO_result_IV.result$ID,
    ...
    rownames(GenePairs_enrichGO_result_IV.result_exper) <- GenePairs_enrichGO_result_IV.result_exper$ID
    
    GO_term_exper <- enrichGO_result_IV.result[which(GO_pvalue_exper < 0.05),]
    GenePairs_GO_term_exper <- GenePairs_enrichGO_result_IV.result_exper[which(GO_pvalue_exper < 0.05),]
    GenePairs_GO_term_exper <- GenePairs_GO_term_exper[order(GenePairs_GO_term_exper$pvalue),]
    save(GO_term_exper,GenePairs_GO_term_exper,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_iv/",project,"_gene_pairs_GOterms_exper_085_stageiv_up.Rdata",sep = ""))
  ...
  write.xlsx(Genepairs_xlsx_exper_result,file = paste("./main6_enrichGO_results_Final/085/",pcc_cutoff_item,"/","stage_iv/","List_gene_pairs_GOterms_exper_085_stageiv_up.xlsx",sep = ""))
